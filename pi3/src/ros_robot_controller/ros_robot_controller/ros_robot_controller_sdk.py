...
class PacketControllerState(enum.IntEnum):
    # 通信协议的格式(Format of the communication protocol)
...
class PacketFunction(enum.IntEnum):
    # 可通过串口实现的控制功能(Control functions that can be implemented via serial port)
    PACKET_FUNC_SYS = 0
    PACKET_FUNC_LED = 1  # LED控制（LED control）
    PACKET_FUNC_BUZZER = 2  # 蜂鸣器控制(Buzzer control)
    PACKET_FUNC_MOTOR = 3  # 电机控制(Motor control)
    PACKET_FUNC_PWM_SERVO = 4  # PWM舵机控制, 板子上从里到外依次为1-4(PWM servo control; on the board, the sequence from inner to outer is 1 to 4)
    PACKET_FUNC_BUS_SERVO = 5  # 总线舵机控制(Serial bus servo control)
    PACKET_FUNC_KEY = 6  # 按键获取(Get the button)
    PACKET_FUNC_IMU = 7  # IMU获取(Get the IMU)
    PACKET_FUNC_GAMEPAD = 8  # 手柄获取(Get the handle)
...
class PacketReportKeyEvents(enum.IntEnum):
    # 按键的不同状态(Different status of buttons)
...
class Board:
    ...
    def __init__(self, device="/dev/rrc", baudrate=1000000, timeout=5):
        ...
        # 队列用来存储数据(The queue is used to store data)
    ...
    def get_battery(self):
        # 获取电压，单位mAh(Obtain the voltage, in units of mAh)
        if self.enable_recv:
            try:
    ...
    def get_button(self):
        # 获取按键key1， key2状态，返回按键ID(1表示按键1，2表示按键2)和状态(0表示按下，1表示松开)(Retrieve the status of buttons key1 and key2, and return the button ID (1 for button 1, 2 for button 2) and the status (0 for pressed, 1 for released))
        if self.enable_recv:
            try:
    ...
    def get_imu(self):
        # 获取IMU数据，返回ax, ay, az, gx, gy, gz(Obtain IMU data, returning ax, ay, az, gx, gy, gz)
        if self.enable_recv:
            try:
                # ax, ay, az, gx, gy, gz
    ...
    def set_oled_text(self, line, text):
        data = [line, len(text)] # 子命令为 0x01 设置 SSID, 第二个字节是字符串长度，该长度包含'\0'字符串结束符(Subcommand 0x01 sets the SSID; the second byte is the string length, which includes the '\0' null-terminator)
