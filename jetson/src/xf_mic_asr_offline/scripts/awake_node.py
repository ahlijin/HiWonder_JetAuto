...
# @Author: Aiden
# @Date: 2023/11/16
# 环形麦克风阵列Python SDK(Delay to wait for other nodes to start up properly)
import re
import time
import json
import rclpy
...
class CircleMic:
    ...
    def switch_mic(self, mic="mic6_circle"):
        # mic：麦克风阵列类型，mic4：线性4麦，mic6：线性6麦， mic6_circle：环形6麦(mic: Microphone array type; mic4: Linear 4-microphone array; mic6: Linear 6-microphone array; mic6_circle: Circular 6-microphone array)
        param ={
            "type": "switch_mic",
            "content": {
                "mic": "mic6_circle"
    ...
    # 唤醒词更换（浅定制）(Wake word change (light customization))
    ...
    # 数据串口发送(Send data through serial port)
    def send_data(self, header, args):
        ...
        self.serialHandle.write(packet)  # 发送主控消息(Send main-control info)
        ...
        while True:
            ...
            if recv_data == header_[0]:
                ...
                if recv_data == header_[1]:
                    ...
                    else:  # 没有收到确认(Not receive the confirmation info)
                        recv_data = self.serialHandle.read(4)
                        self.serialHandle.read((recv_data[1] << 8 | recv_data[0]) + 1)
    ...
    # 检测是否唤醒以及唤醒对应角度(Detect whether the wake word is triggered and the corresponding angle of the wake word)
